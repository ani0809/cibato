...
const AdminDemoEdit = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const endpoint = isNew ? '/demos' : `/demos/${id}`;
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center gap-4">
                <button onClick={() => navigate('/admin/demos')} className="p-2 hover:bg-slate-100 rounded-lg">
                    <ArrowLeft className="w-5 h-5 text-slate-500" />
                </button>
                <h1 className="text-2xl font-bold text-slate-800">{isNew ? 'New Demo' : 'Edit Demo'}</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">Demo Title</label>
                            <input
                                required
                ...
                <div className="flex justify-end pt-4 border-t border-slate-100">
                    ...
                    >
                        ...
                        <span>{loading ? 'Saving...' : 'Save Demo'}</span>
                    </button>
                </div>
            </form >
        </div >
